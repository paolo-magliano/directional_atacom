...
def main():
    ...
    if n_record:
        launcher_record = Launcher(f'{env}_record', f"experiment", n_seeds=n_record, start_seed=0, memory_per_core=8000, n_cores=1,
                        conda_env="d_atacom", hours=10, n_exps_in_parallel=n_record, partition="stud", gres='gpu:rtx2080:1')
    if n_seeds - n_record > 0:
        launcher = Launcher(env, f"experiment", n_seeds=n_seeds-n_record, start_seed=n_record, memory_per_core=2000, n_cores=1,
                        conda_env="d_atacom", hours=10, n_exps_in_parallel=max(n_exp_in_parallel-n_record, 1), partition="stud")
