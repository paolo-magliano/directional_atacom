...
def merge_config(base, new):
    """Merge `new` into `base`: nested dicts are merged, everything else is overridden."""
...
def load_configs(*paths):
    """Load yaml files in order, later files overriding earlier ones."""
    config = {}
    for path in paths:
        with open(path) as f:
            merge_config(config, yaml.safe_load(f) or {})
    return config
...
def main():
    ...
    env = "air_hockey_vel"
    # Choices: sac, atacom_sac, atacom_sac_dc, datacom_sac, datacom_sac_dc
    alg = "atacom_sac_dc"
    ...
    config = load_configs(os.path.join("configs", "defaults", "defaults.yaml"), *configs)
